...
from makers.routers.api import core_router
from makers.config.settings import settings
from makers.config.log_config import conf, set_datetime
from makers.utils.exceptions import not_found_exception_handler
...
api_app.include_router(
    core_router, prefix=settings.CURRENT_API_VERSION, tags=["api_v1"]
)


#: Основной FastAPI инстанс
app = FastAPI(
    title=f"{settings.PROJECT_NAME} root app", openapi_url=None, docs_url=None
)
...
class ExceptionMiddleware(BaseHTTPMiddleware):
    """Middleware for catching errors in the backend response"""
